...
public class BuildingBuilder {
			...
			for (int x = 0; x < clipboard.getWidth(); x++) {
				for (int z = 0; z < clipboard.getLength(); z++) {
					for (int y = 0; y < clipboard.getHeight(); y++) {
						BaseBlock base = clipboard
								.getPoint(new Vector(x, y, z));
						if (base != null) {
			...
			for (int z = 0; z < clipboard.getLength(); z++) {
				for (int y = 0; y < clipboard.getHeight(); y++) {
					...
					if (base != null) {
						int type = base.getType();
						int data = base.getData();
						Block block = world.getBlockAt(basex + x, basey + y,
								basez + z);
						...
						case 3:
							...
							break;
						case 2:// GRASS
							stage1.add(new PendingBlock(block, type,
									(byte) data));
							break;
						case 17:// LOG
							stage2.add(new PendingBlock(block, type,
									(byte) data));
							break;
						case 5:// WOOD
							stage3.add(new PendingBlock(block, type,
									(byte) data));
							break;
						case 98:// STONE BRICK
						case 35:
							stage4.add(new PendingBlock(block, type,
									(byte) data));
							break;
						case 47:// BOOKSHELF
						case 85:// FENCE
						...
						case 84:// JUKEBOX
							stage5.add(new PendingBlock(block, type,
									(byte) data));
							break;
						...
						case 20:
						case 102:
							stage6.add(new PendingBlock(block, type,
									(byte) data));
							break;
						case 63:
						case 64:
						case 71:
						case 355:
							stage8.add(new PendingBlock(block, type,
									(byte) data));
							break;

						default:
							stage7.add(new PendingBlock(block, type,
									(byte) data));
							break;
	...
	public void PlaceNextBlock() {
		...
		block.setTypeId(type);
		block.setData((byte) data);
		System.out.println("Max Z " + clipboard.getWidth() + " Y"
				+ clipboard.getHeight());
	}
	...
	public void StartSlowpaste() {
		schedulerID = plugin.getServer().getScheduler()
